    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->outputTextEdit->setOpenExternalLinks(true);
    ui->outputTextEdit->setTextInteractionFlags(Qt::TextBrowserInteraction);
    ...
    for (const Movie& m : movieDB) {
        if (m.genre == genre && m.mood == mood && m.timeSlot == time) {
            QString html = QString(R"(
                ...
                <p>📝 %3</p>
                <p><a href="%4">▶ Watch Trailer</a></p>
            )").arg(m.title, m.imagePath, m.description, m.trailerUrl);
